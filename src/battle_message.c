...
const u8 *const gBattleStringsTable[STRINGID_COUNT] =
{
    ...
    [STRINGID_WALLYUSEDITEM]                        = COMPOUND_STRING("Lucas used\none {B_LAST_ITEM}!"), //no decapitalize until it is everywhere
    ...
    [STRINGID_YOUTHROWABALLNOWRIGHT]                = COMPOUND_STRING("All right! I got it's HP down!\nTime to throw a Poké Ball!"),
...
const u8 gText_WhatWillLucasDo[] = _("What will\n{B_PLAYER_MON1_NAME} do?");
...
static const u8 *BattleStringGetOpponentNameByTrainerId(u16 trainerId, u8 *text, u8 multiplayerId, enum BattlerId battler)
{
    ...
    else
    {
        ...
        if (trainerClass == TRAINER_CLASS_RIVAL ||trainerClass == TRAINER_CLASS_RIVAL_EARLY_FRLG || trainerClass == TRAINER_CLASS_RIVAL_LATE_FRLG || trainerClass == TRAINER_CLASS_CHAMPION_FRLG)
            toCpy = GetExpandedPlaceholder(PLACEHOLDER_ID_BARRY);
        else
